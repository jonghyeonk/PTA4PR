...
def argmin_levenshtein_v2(stru: str, list_stri: List[str]) -> Union[str, None]:
    """
    Given a string (stru), finds a string in a list
    of strings (list_stri) that minimizes the Levenshtein distance.
    ...
    if list_stri:
        ...
        # to the provided string
        list_stri = sorted(list_stri, key=lambda x: abs(len(x) - len_stru))
        argmin_dist = list_stri
        min_edit_dist = list(map(lambda x: levenshtein(x, stru), list_stri ))
        return argmin_dist, min_edit_dist
    return None
