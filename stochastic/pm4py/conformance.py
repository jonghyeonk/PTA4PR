...
def conformance_diagnostics_alignments_v2(log: EventLog, *args, multi_processing: bool = False) -> List[Dict[str, Any]]:
    """
    Apply the alignments algorithm between a log and a process model.
    The methods return the full alignment diagnostics.
    ...
    if len(args) == 3:
        if type(args[0]) is PetriNet:
            ...
            if multi_processing:
                ...
                return alignments.apply_v2(log, args[0], args[1], args[2], parameters=get_properties(log))
        elif type(args[0]) is dict or type(args[0]) is Counter:
            # DFG alignments
            from pm4py.algo.conformance.alignments.dfg import algorithm as dfg_alignment
            return dfg_alignment.apply(log, args[0], args[1], args[2], parameters=get_properties(log))
    ...
    if multi_processing:
        ...
        return alignments.apply_v2(log, net, im, fm, parameters=get_properties(log))
